...
export class OrderStore {
    ...
    @action.bound loadOrderProducts(order_id: string) {
        ...
        axiosInstane.post("/waiter/order", data)
        .then((response) => {
    ...
    @action.bound cancelOrder(order_id: string, history) {
        const data = {order_id: order_id}
        axiosInstane.post("/waiter/order/cancel", data)
        .then((response) => {
            if (response.data.success) {
    ...
    @action.bound markOrderAsPaid(order_id: string, history) {
        const data = {order_id: order_id}
        axiosInstane.post("/waiter/order/paid", data)
        .then((response) => {
            if (response.data.success) {
